...
public class JedisUtils {
	...
	static {
		...
		// redis未设置了密码：139.224.49.192
		//jedisPool = new JedisPool(config, "139.224.49.192",6379,10000,"jclm@2017");
		
				System.out.println("redis未设置了密码");
				String redishost = Global.getConfig("redishost");
				String redisport = Global.getConfig("redisport");
				String redispasswd = Global.getConfig("redispasswd");
				//jedisPool = new JedisPool(config, "118.178.224.247",6379,10000,"jclm@2017");
				jedisPool = new JedisPool(config,redishost,Integer.parseInt(redisport),10000,redispasswd);
	}
	...
	public class Hash {
		...
		public Map<byte[], byte[]> hgetAllByte(String key) {
			Jedis sjedis = getJedis();
			byte[] byteKey = Encode.encode(key);
			Map<byte[], byte[]> map = sjedis.hgetAll(byteKey);
			returnJedis(sjedis);
			return map;
		}
		...
		public long hset(String key, String fieid, byte[] value) {
			Jedis jedis = getJedis();
			long s = jedis.hset(Encode.encode(key), Encode.encode(fieid), value);
			returnJedis(jedis);
			return s;
		}
		 ...
		 * @return List<String>
		 */
		public List<byte[]> hvalsByte(String key) {
			Jedis sjedis = getJedis();
			byte[] byteKey = Encode.encode(key);
			List<byte[]> list = sjedis.hvals(byteKey);
			returnJedis(sjedis);
			return list;
		}
	...
	public static void main(String[] args) {
		...
		JedisUtils.Strings strings = jedisUtils.new Strings();
		strings.get("foo");
		System.out.println("-----" + strings.get("foo"));
		...
		for (; i < len; i++) {
			dbean = new DictionarysCacheBean();
			dbean.setCode(new StringBuffer().append("0").append(i + 1).toString());
			dbean.setName("xxx" + i);
			hash.hset(CacheKeys.SERVICE_PRICE, dbean.getCode(), Encode.encode(dbean));
		}

		List<byte[]> list = hash.hvalsByte(CacheKeys.SERVICE_PRICE);
		List<DictionarysCacheBean> relist = com.google.common.collect.Lists.newArrayList();
		for (byte[] by : list) {
			dbean = (DictionarysCacheBean) unserizlize(by);
			relist.add(dbean);
			System.out.println(dbean.getName());
		}
		Collections.sort(relist);
		for (DictionarysCacheBean by : relist) {
			System.out.println(by.getName());
		}

		Map<byte[], byte[]> result = hash.hgetAllByte(CacheKeys.SERVICE_PRICE);
		dbean = (DictionarysCacheBean) unserizlize(result.get(Encode.encode("01")));
		System.out.println(dbean.getName());
		System.out.println(hash.hget(Encode.encode(CacheKeys.SERVICE_PRICE), Encode.encode("01")));*/
	}
